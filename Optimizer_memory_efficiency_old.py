...
class Optimizer_memory_efficiency(object):
    def __init__(self, ys, xs, grad_ys=None,
                 debug=False,
                 debug_level=1,
                 cpu_device="/cpu:0", **kwargs):
    ...
    def run(self):
        ...
        if xs_intersect_checkpoints:
            self._log_info("Warning, some input nodes are also checkpoint nodes: %s".format(
                xs_intersect_checkpoints))
        ys_intersect_checkpoints = set(self._ys).intersection(set(checkpoints))
        self._log_info("ys: %s, checkpoints: {}, intersect: {}".format(self._ys, checkpoints,
        ...
        if ys_intersect_checkpoints:
            self._log_info("Warning, some output nodes are also checkpoints nodes: {}".format(
                self.format_ops(ys_intersect_checkpoints)))
        ...
        ops_to_copy = self.fast_backward_ops(seed_ops=[y.op for y in self._ys],
                                             stop_at_ts=checkpoints, within_ops=self.fwd_ops)
        self._log_info("Found %s ops to copy within fwd_ops {}, seed {}, stop_at {}".format(
            len(ops_to_copy), self.fwd_ops, [r.op for r in self._ys], checkpoints), 1)
        ...
        self._log_info("Rewired %s in place of {} restricted to {}".format(
        ...
        dv = tf_gradients(ys=copied_ys, xs=boundary + self._xs, grad_ys=self._grad_ys, **self._kwargs)
        ...
        checkpoints_sorted_lists = self.tf_toposort(checkpoints, within_ops=self.fwd_ops)
        for ts in checkpoints_sorted_lists[::-1]:
            ...
            ops_to_copy = self.fast_backward_ops(within_ops=self.fwd_ops, seed_ops=[r.op for r in ts],
            ...
            dv = tf_gradients(boundary,
                              ...
                              grad_ys=substitute_backprops, **self._kwargs)
            self._log_info("Got gradients {}".format(dv), 1)
            self._log_info("for {}".format(boundary), 1)
            self._log_info("with respect to {}".format(checkpoints_disconnected_other + self._xs), 1)
            self._log_info("with boundary backprop substitutions {}".format(substitute_backprops), 1)
    ...
    def _get_checkpoint(self):
        ...
        for ts in [ts_filtered, ts_all]:
            ...
            bottleneck_ts = []
            for t in ts:
                b = set(ge.get_backward_walk_ops(t.op, inclusive=True, within_ops=self.fwd_ops))
                f = set(ge.get_forward_walk_ops(t.op, inclusive=False, within_ops=self.fwd_ops))
                ...
                if not set(b_inp).intersection(f_inp) and len(b_inp) + len(f_inp) >= len(ts_all):
                    ...
                    self._log_info("Rejected bottleneck candidate and ops {}".format(
                        [t] + list(set(ts_all) - set(b_inp) - set(f_inp))), 3)
        ...
        bottlenecks_sorted_lists = self.tf_toposort(bottleneck_ts, within_ops=self.fwd_ops)
    ...
    def my_add_control_inputs(self, wait_to_do_ops, inputs_to_do_before):
...
def gradients_ome(ys, xs, grad_ys=None, **kwargs):
    if not isinstance(ys, list):
        ys = [ys]
    if not isinstance(xs, list):
        xs = [xs]
    ome = Optimizer_memory_efficiency(ys, xs, grad_ys, **kwargs)
    return ome.run()
    # return gradients(ys, xs, grad_ys, checkpoints='ome', **kwargs)
